...
{
    const bool animated =
        params.contains( QStringLiteral( "animateRollout" ), Qt::CaseInsensitive );
    const bool inOwnWindow = params.contains( QStringLiteral( "ownWindow" ), Qt::CaseInsensitive );
    const bool isCustomSize = params.contains( QStringLiteral( "isCustomSize" ), Qt::CaseInsensitive );
    ...
    _positioner.reset( createPositioner( inOwnWindow, noScroll, screen_idx, isCustomSize ) );
    _positioner->enableAnimation( animated );

    connect(
        qGuiApp, &QGuiApplication::applicationStateChanged, this, []( Qt::ApplicationState s ) {
...
void OpenVirtualKeyboardInputContext::setFocusObject( QObject* object )
{
    ...
    if ( QQuickItem* item = qobject_cast<QQuickItem*>( object ) ) {
        ...
        if ( typeName.startsWith( "TextField_QMLTYPE" )
             || typeName.startsWith( "TextArea_QMLTYPE" ) ) {
...
void OpenVirtualKeyboardInputContext::updateEnterKeyAction()
{
    ...
    if ( valid )
        enterKeyActionConnection =
            connect( item, enterKeyActionChangedSignal( item ), this, enterKeyActionChangedSlot() );
...
QMetaMethod OpenVirtualKeyboardInputContext::enterKeyActionChangedSignal( QObject* object ) const
{
    if ( !object )
        return QMetaMethod {};
...
{
    if ( inOwnWindow )
        return new KeyboardWindowPositioner( screen, isCustomSize );
